...
public extension Swift.Array where Element == NSMutableAttributedString {
    mutating func applyListItemStylingIfNecessary(node: Node, context: [CodingUserInfoKey: Any]) {
        ...
        if listContext.isFirstListItemChild {
            ...
            attributedString.addAttributes(
                [.foregroundColor: rendererConfiguration.textConfiguration.textColor],
    ...
    private func applyListItemStyling(
        ...
        let indentationInPx = CGFloat(listContext.level - 1) * rendererConfiguration.textList.indentationMultiplier + 0.001 // WORKAROUND: In Contentful, lists are indented so that they're not aligned to their previous nodes. That's not the case with Pharma Rich Text. ist's bullet points are aligned with the the lists previous node.
