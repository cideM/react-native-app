...
class Unused
  ...
  def find_usages_in_files(files, xibs, items_in)
    ...
    files.each { |file|
      ...
      lines = all_lines.map { |line| line.split("//").first }
      comments = all_lines.filter { |line| line.include? "//" }.map { |line| "//" + line.split("//").last }
      ignores += comments.filter { |line| line.include? "unused:ignore" }.map { |line| line.split(" ").last }
    ...
    items = items.filter { |item| !ignores.include? item.name }
